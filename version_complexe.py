...
def add_person_ui(people_dict):
    with st.form("Add Person"):
        ...
        job = st.text_input("What's your domain of work?")
...
st.text("If you plan to use a JSON, please follow the format while replacing '' by "": ")
st.html("<p>{'people': [{'first_name': 'Pou', 'last_name': 'Let', 'age': 32, 'sex': 'M', 'hobby': 'Volleyball', 'job': 'IT', 'met_at': 'HS', 'connections': ['Alice A', 'Anais V']}, {'first_name': 'Ro', 'last_name': 'Lex', 'age': 32, 'sex': 'M', 'hobby': 'Volleyball', 'job': 'Sales', 'met_at': 'HS', 'connections': ['Alice A', 'Anais V']}]}</p>")

grouping_option = st.selectbox("Group nodes by", ["None", "met_at", "hobby", "job"])
...
if people_dict:
    # Collect available hobbies and jobs
    hobbies = sorted(set(p.hobby for p in people_dict.values() if p.hobby))
    jobs = sorted(set(p.job for p in people_dict.values() if p.job))

    col1, col2 = st.columns(2)
    with col1:
        hobby_filter = st.selectbox("Filter by hobby", ["All"] + hobbies)
    with col2:
        job_filter = st.selectbox("Filter by job", ["All"] + jobs)

    # Apply filters
    filtered_people = {
        name: p for name, p in people_dict.items()
        if (hobby_filter == "All" or p.hobby == hobby_filter)
        and (job_filter == "All" or p.job == job_filter)
    }

    if filtered_people:
        fig = draw_graph(filtered_people, group_by=grouping_option)
        ...
        st.warning("No people match your filter.")
